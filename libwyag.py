...
# the path where to init the repo. It defaults to .
...
#Bridge function
def cmd_init(args):
    # Bridge function to inizializate a new repository.
...
#object wrapper for a single record
...
#parser which just calls the previous one in a loop, until input data is exhausted
def tree_parse(raw):
    pos = 0
    max = len(raw)
    ret = list()
...
#sort items and write them in order
...
#We don’t need to implement tag objects, we can reuse GitCommit and just change the fmt field
